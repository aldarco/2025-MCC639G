...
private:
	...
	using Node       = LLNode<T>;
	using iterator   = forward_linkedlist_iterator<T>;
	using Container  = CLinkedList<T>;

	Container *m_pList = nullptr;
	Node      *m_pNode = nullptr;

public:
	forward_linkedlist_iterator(Container *pList, Node *pNode) : m_pList(pList), m_pNode(pNode){}
	    forward_linkedlist_iterator& operator=(const forward_linkedlist_iterator &other) {
        m_pList = other.m_pList;
        m_pNode = other.m_pNode;
        return *this;
    }
    
    // comparacion y rerencia)
